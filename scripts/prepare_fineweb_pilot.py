...
def main():
    ...
    fineweb_revision = api.dataset_info(
        FINEWEB_REPO, revision=FINEWEB_REVISION
    ).sha
    if fineweb_revision != FINEWEB_REVISION:
        raise RuntimeError(
            f"FineWeb resolved to {fineweb_revision}, expected {FINEWEB_REVISION}"
        )
    ...
    if not parquet_files:
        raise RuntimeError(f"no Parquet files found under {FINEWEB_PATH}")
        ...
        for source_file in parquet_files:
            ...
            for batch in parquet.iter_batches(
                ...
                for offset in range(batch.num_rows):
                    ...
                    fineweb_id = columns["id"][offset]
                    text = columns["text"][offset]
                    fineweb_token_count = columns["token_count"][offset]
                    if not isinstance(fineweb_id, str) or not fineweb_id:
                        raise RuntimeError(
                            f"invalid FineWeb id at {source_file}:{source_row}"
                        )
                    if fineweb_id in seen_ids:
                        raise RuntimeError(f"duplicate FineWeb id: {fineweb_id}")
                    seen_ids.add(fineweb_id)
                    if not isinstance(text, str) or not text:
                        raise RuntimeError(
                            f"invalid text at {source_file}:{source_row}"
                        )
                    if isinstance(fineweb_token_count, bool) or not isinstance(
                        fineweb_token_count, Integral
                    ):
                        raise RuntimeError(
                    ...
                    if not token_ids or min(token_ids) < 0 or max(token_ids) >= GPT2_VOCAB_SIZE:
                        raise RuntimeError(
                    ...
                    is_validation = (
                        int.from_bytes(
                            hashlib.sha256(fineweb_id.encode("utf-8")).digest(), "big"
                        )
                        % VALIDATION_MODULUS
                        == 0
                    )
                    ...
                    entry = {
                        ...
                        "fineweb_minus_tokenizer_token_count": (
                            int(fineweb_token_count) - len(token_ids)
                        ),
                        "fineweb_token_count": int(fineweb_token_count),
    ...
    if not target_reached:
        raise RuntimeError("FineWeb source ended before the pilot target")
    ...
    fineweb_token_count = sum(entry["fineweb_token_count"] for entry in entries)
    tokenizer_token_count = sum(entry["tokenizer_token_count"] for entry in entries)
    token_count_mismatch_documents = sum(
        entry["fineweb_token_count"] != entry["tokenizer_token_count"]
        for entry in entries
    )
    token_count_absolute_drift = sum(
        abs(entry["fineweb_minus_tokenizer_token_count"]) for entry in entries
    )
    ...
    summary = {
        "artifacts": artifacts,
        "counts": {
            "fineweb_minus_tokenizer_tokens": (
                fineweb_token_count - tokenizer_token_count
            ),
            "fineweb_token_count": fineweb_token_count,
        ...
        "fineweb": {
            ...
            "token_count_semantics": (
                "advisory pre-PII metadata; selection uses recomputed tokenizer IDs"
            ),
        },
        "format": {
            "append_eod": GPT2_EOD,
        ...
        "selection": {
            ...
            "validation_rule": "int.from_bytes(sha256(id_utf8), 'big') % 1000 == 0",
        },
        "tokenizer": {
            "eod": GPT2_EOD,
