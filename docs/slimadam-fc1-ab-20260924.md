...
Both te4 save/resume smokes passed (12→14), including the runtime FC1 assignment contract and exact extra split-state storage of 4,530,176 bytes. Neither 2254-step training started: the immediate post-cleanup NFS free-space readings were below 22 GiB. Subsequent audit showed delayed reclamation, but shared NFS2 remained around 20 GiB.

Prepared recovery option, requiring approval to publish checkpoints: set `SLIM_AB_ARCHIVE_HF=1` and supply the existing scoped `HF_TOKEN`, then submit `train baseline` and `train split` on te4. Checkpoints live on task-local `/tmp` (40 GiB minimum); logs remain on the variant's persistent volume (1 GiB minimum). Each 322-step checkpoint is hard-linked into a staging snapshot, uploaded to `AverageMetaheuristicsEnjoyer/hmoe-stage3-checkpoints/slimadam-fc1-ab-20260924-v1/<variant>/iter_XXXXXXX`, and remotely verified by file size and SHA-256 before publishing `resume.json`. The previous rolling remote checkpoint is removed only after its replacement verifies. Final iteration 2254 is retained. Resubmission downloads and verifies the latest archived state before continuing; prior train logs without a verified archive prevent silent fresh restart. Completed smoke evidence is reused by direct `train` mode. This follows the archive/restore scheme already used by the corrected Stage 3 full runs.
